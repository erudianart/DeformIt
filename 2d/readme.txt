   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_GenerateDeformedAndSegmentedImages.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* GenerateDeformedAndSegmentedImages.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* GenerateDeformedAndSegmentedImages.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* GenerateDeformedAndSegmentedImages (application created by MATLAB Compiler)
		* run_GenerateDeformedAndSegmentedImages.sh (shell script that can be run 
	...
	All Platforms:            
		* GenerateDeformedAndSegmentedImages.so (shared library, file extension 
		  varies by platform)
		* GenerateDeformedAndSegmentedImages.h (library header file)
		* GenerateDeformedAndSegmentedImages.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* GenerateDeformedAndSegmentedImages.ctf (Component Technology File archive)	
	* GenerateDeformedAndSegmentedImages.xml (documentation files)
	* GenerateDeformedAndSegmentedImages.pdb (program database file - if 
	  DEBUG option selected)
	* GenerateDeformedAndSegmentedImages.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* GenerateDeformedAndSegmentedImages.ctf (Component Technology File archive)
	* GenerateDeformedAndSegmentedImages_1_0.dll (component containing 
	  compiled M-code)
	...
	* GenerateDeformedAndSegmentedImages.jar (Java archive containing Java 
	  interface to M-code in GenerateDeformedAndSegmentedImages.ctf)
	* GenerateDeformedAndSegmentedImages.ctf (Component Technology File archive,
	  platform-dependent on target machine)
